...
namespace DigitalRune.Windows.TextEditor.Folding
{
  ...
  public class XmlFoldingStrategy : IFoldingStrategy
  {
    ...
    /// </summary>
    /// <value>
    /// <c>true</c> if attributes should be displayed on folded elements; otherwise, <c>false</c>.
    /// </value>
    public bool ShowAttributesWhenFolded
    {
      get { return showAttributesWhenFolded; }
      set { showAttributesWhenFolded = value; }
    }
    ...
    /// Initializes a new instance of the <see cref="XmlFoldingStrategy"/> class.
    /// </summary>
    /// <param name="showAttributesWhenFolded">if set to <c>true</c> attributes will be displayed on folded elements.</param>
    public XmlFoldingStrategy(bool showAttributesWhenFolded)
    {
      this.showAttributesWhenFolded = showAttributesWhenFolded;
    }
    ...
    void CreateCommentFold(IDocument document, List<FoldMarker> foldMarkers, XmlTextReader reader)
    {
      if (reader.Value != null)
      {
        ...
        if (lines.Length > 1)
        {
          ...
          FoldMarker foldMarker = new FoldMarker(document, startLine, startCol, endLine, endCol, foldText);
    ...
    void CreateElementFold(IDocument document, List<FoldMarker> foldMarkers, XmlTextReader reader, XmlFoldStart foldStart)
    {
      ...
      if (endLine > foldStart.Line)
      {
        ...
        FoldMarker foldMarker = new FoldMarker(document, foldStart.Line, foldStart.Column, endLine, endCol, foldStart.FoldText);
        foldMarkers.Add(foldMarker);
      }
    }
